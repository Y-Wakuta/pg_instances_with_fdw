...
--result
----------------------------------------------------
Hash Join  (cost=227.21..297.32 rows=1185 width=294) (actual time=1.225..1.230 rows=5 loops=1)
  Hash Cond: ((vmg.title)::text = (vr.title)::text)
  ->  Foreign Scan on movie_genres vmg  (cost=100.00..127.55 rows=585 width=116) (actual time=0.733..0.734 rows=5 loops=1)
  ->  Hash  (cost=122.15..122.15 rows=405 width=178) (actual time=0.441..0.441 rows=20 loops=1)
        Buckets: 1024  Batches: 1  Memory Usage: 10kB
        ->  Foreign Scan on reviews vr  (cost=100.00..122.15 rows=405 width=178) (actual time=0.426..0.430 rows=20 loops=1)
...
--result
----------------------------------------------------
Foreign Scan  (cost=102.50..128.49 rows=200 width=58)
  Output: nationality
  Relations: Aggregate on (public.actor)
  Remote SQL: SELECT nationality FROM public.actor GROUP BY nationality
...
Foreign Scan  (cost=102.50..128.49 rows=200 width=58)
  Output: r.review
  Relations: Aggregate on (public.reviews r)
  Remote SQL: SELECT review FROM public.reviews GROUP BY review
